...
the existing stack + the validator suite," not "built."

**Grounding pass (re-verified against live `src/data/` via `node`, 2026-07-14):**
`ALL_CARDS` (see `src/data/cards.js`; free deck `CARDS` excludes mature + quarantined);
...
`DATING_REVIEW_COMPLETE` (true since the 2026-07-16 sign-off). Counts move — read them
from the exports rather than from this line; the
...
**Why a separate `situations.js` map and not a field edit:** editing every card to
...
- **`ALL_CARDS.length` cards** total (do not hardcode; it moves). Phrases (`p`) exist **only** in stage
...
Mirror the existing `DATING_REVIEW_COMPLETE` flag (`datingContent.js`) with a
...
4. **Situation tag: side map vs. card field.** This spec uses a separate
   `src/data/situations.js` `SITUATION_TAGS` map to avoid editing the existing cards
